...
def parse_args():
    p = argparse.ArgumentParser(description="Monga v3 balanced memory-grounding SFT with QLoRA")
    p.add_argument("--model", default="Qwen/Qwen3-0.6B")
    p.add_argument("--output-dir", default="finetune/runs/qwen3-0.6b-memory-sft-v3")
...
def main():
    ...
    print(f"v3 train examples: {train_count}")
    print(f"v3 validation examples: {validation_count}")

    if train_count != 84:
        raise RuntimeError(f"Expected 84 v3 train examples, got {train_count}.")
    if validation_count != 18:
        raise RuntimeError(f"Expected 18 v3 validation examples, got {validation_count}.")
