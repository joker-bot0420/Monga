...
def parse_args():
    p = argparse.ArgumentParser(
        description="Monga v5 temporal-query memory-grounding SFT with QLoRA"
    )
    p.add_argument("--model", default="Qwen/Qwen3-0.6B")
    p.add_argument(
        "--output-dir",
        default="/content/monga-qwen3-0.6b-memory-sft-v5",
    )
...
def main():
    ...
    dataset = load_dataset(
        ...
        data_files={
            "train": [
                ...
                str(data_root / "train_temporal_v5.jsonl"),
            ],
            "validation": [
                ...
                str(data_root / "validation_v5.jsonl"),
            ],
        },
    )
    ...
    print(f"v5 train examples: {train_count}")
    print(f"v5 validation examples: {validation_count}")

    if train_count != 120:
        raise RuntimeError(
            f"Expected 120 v5 train examples, got {train_count}."
        )
    if validation_count != 34:
        raise RuntimeError(
            f"Expected 34 v5 validation examples, got {validation_count}."
        )
    ...
    def to_prompt_completion(example):
        ...
        if len(messages) < 2 or messages[-1].get("role") != "assistant":
            raise ValueError(
                "Each training example must end with one assistant message."
            )
        return {
            "prompt": messages[:-1],
