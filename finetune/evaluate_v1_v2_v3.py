...
def parse_args():
    p = argparse.ArgumentParser(description="Compare Monga v1, v2, and v3 adapters on 32 held-out prompts.")
    ...
    p.add_argument("--output", default="/content/monga-v1-v2-v3-comparison.json")
...
def main():
    args = parse_args()
    rows = load_rows([args.heldout, args.heldout_no_memory, args.heldout_v3])

    if len(rows) != 32:
        raise RuntimeError(f"Expected 32 held-out examples, got {len(rows)}.")
    ...
    for row in rows:
        item = {
            ...
            "v1": results["v1"][row["id"]],
            "v2": results["v2"][row["id"]],
    ...
    print("\n=== V1 VS V2 VS V3 HELD-OUT COMPARISON ===")
