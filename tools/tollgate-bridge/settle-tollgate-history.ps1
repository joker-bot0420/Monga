...
function Get-ProductionManifest{return Get-ProductionHistoricalEvidenceManifest}
...
try{
    ...
    $orchestratorLockPath=Join-Path $state 'orchestrator/orchestrator.lock';$taskLockPath=Join-Path $state "orchestrator/task-$($constants.ApprovalCommentId).lock";Assert-NoReparsePath $state $orchestratorLockPath;Assert-NoReparsePath $state $taskLockPath
    ...
    $id=$constants.ApprovalCommentId;$completed=Join-Path $state "completed\$id.json";$reported=Join-Path $state "reported\$id.json";if(Test-Path -LiteralPath $completed){throw 'A completed record already exists; refusing historical recovery.'};if((Test-Path -LiteralPath $reported -PathType Leaf)-and-not(Test-Path -LiteralPath (Join-Path $state "failed\$id.json") -PathType Leaf)){throw 'A reported record exists without a historical failed terminal; refusing recovery before lifecycle writes.'}
    ...
    if(Test-Path $auditPath){Assert-ExactArtifact $auditPath $auditBytes {param($v)Assert-HistoricalRecoveryMetadataAgainstManifest $v.recovery $manifest}}else{Write-BytesAtomically $auditBytes $auditPath};if($FailurePoint-eq'AfterAudit'){throw 'SYNTHETIC_FAILURE_AFTER_AUDIT'}
    ...
    if(Test-Path $failedPath){Assert-ExactArtifact $failedPath $terminalBytes {param($v)Assert-HistoricalRecoveryTerminalRecordAgainstManifest $v $manifest}}else{Write-BytesAtomically $terminalBytes $failedPath};if($FailurePoint-eq'AfterTerminal'){throw 'SYNTHETIC_FAILURE_AFTER_TERMINAL'}
    $fresh=Get-ValidatedEvidence $state $manifest;if($fresh.Pending.Sha256-cne$evidence.Pending.Sha256){throw 'Pending changed after validation.'};Assert-ExactArtifact $auditPath $auditBytes {param($v)Assert-HistoricalRecoveryMetadataAgainstManifest $v.recovery $manifest};Assert-ExactArtifact $failedPath $terminalBytes {param($v)Assert-HistoricalRecoveryTerminalRecordAgainstManifest $v $manifest};if((Get-RecoverySha256 ([IO.File]::ReadAllBytes($archivePath)))-cne$evidence.Pending.Sha256){throw 'Archive changed before pending removal.'};if($FailurePoint-eq'BeforePendingRemoval'){throw 'SYNTHETIC_FAILURE_BEFORE_PENDING_REMOVAL'}
